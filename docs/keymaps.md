...
| **Terminal & Harpoon**             | M   |   | **LSP**                            | M   |
|------------------------------------|-----|---|------------------------------------|-----|
| `<leader>to` Terminal split        | n   |   | `K` Hover docs                     | n   |
| `<c-/>` Snacks terminal            | n   |   | `gd` `gD` Def / Decl               | n   |
| `<leader>H` Add to harpoon         | n   |   | `gi` `grt` Impl / TypeDef          | n   |
| `<leader>hh` Quick menu            | n   |   | `grn` Rename                       | n   |
| `<leader>h1-9` Jump to file        | n   |   | `gra` Code action                  | n/x |
|                                    |     |   | `grp` References                   | n   |
| **Format & Lint**                  | M   |   | `<leader>ca` Code action (tiny)    | n/x |
| `<leader>fb` Format buffer         | *   |   | `<leader>ci/co` In/out calls       | n   |
| `<leader>ll` Lint                  | n   |   | `<leader>cR` Rename file           | n   |
| `<leader>lk` Show linters          | n   |   | `<leader>th` Toggle inlay hints    | n   |
...
| `<leader>gP/gp` Preview pop/inline | n   |   | `<leader>gS` Stash                 | n   |
...
| `<leader>Gm/GM` Compare main/br    | n   |   | `<leader>a/A` Swap param →/←       | n   |
...
|                                    |     |   | `<leader>at/af/av` Send this/f/sel |     |
