...
---@param hl string?
---@return string
local function wrap_component(component, fallback_hl, hl)
    if #component == 0 then
        return ""
    end

    hl = hl or fallback_hl
    return table.concat({
...
---@param fallback_hl string
---@return string
local function concat_components(components, fallback_hl)
    local acc = ""
    for _, component in ipairs(components) do
        local rendered = wrap_component(component.component, fallback_hl, component.hl)
        if #rendered > 0 then
            if #acc == 0 then
                acc = rendered
            else
